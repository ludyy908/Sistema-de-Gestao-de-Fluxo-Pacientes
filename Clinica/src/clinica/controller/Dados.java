...
public class Dados {
    ...
    Paciente pac;
    Doenca d;
    Impressoes imp; VerificarDataHora vdh;
    ...
    public void agendarConsulta() throws IOException{
        ...
        if(nome.isBlank() == false){ // se nome for vazio ou em branco vai cancelar a operacao
            ...
            if(data.isBlank() == false){ // se for vazio vai cancelar
                String hora = vdh.verificarHora(nome, data,lista);//verificar a hora na agenda do medico em funcao da data. Se 
                if (hora.isBlank() == false){  // se for vazio vai cancelar. se tudo for aprovado, vai meter nos dados da consulta.
                    cons.setData(data); 
                    cons.setHora(hora);
   ...
   public void lerDados(File file) {
        try{
    ...
    public void gravarDados(File file) {
        try{
